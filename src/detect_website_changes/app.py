...
if os.environ['Stage'] == 'local':
    ...
    s3_cache = cache.S3Cache(session.resource('s3', endpoint_url='http://localstack:4572').Bucket(monita_bucket), 'data/website/')
    ...
    s3_cache = cache.S3Cache(session.resource('s3').Bucket(monita_bucket), 'data/website/')
